  ...
  def create
		...
		if @inquiry.valid?
			unless @inquiry.use_v2.blank?
				unless Kalyce::Recaptcha.verify_recaptcha_v2?(params['g-recaptcha-response'], 'contact')
				...
				unless Kalyce::Recaptcha.verify_recaptcha?(params[:recaptcha_token], 'contact')
					flash[:alert] = t('global.recaptcha_failed')
					@show_recaptcha_v2 = true
				else
					create_data
